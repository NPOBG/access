...
app.post('/api/access-codes', async (req, res) => {
    try {
        const { code, user, unit, isAdmin, activation, deactivation, createdBy } = req.body;
        
        // Validate required fields
        if (!code || !user || !unit || typeof isAdmin === 'undefined' || !activation || !createdBy) {
            return res.status(400).json({ 
                error: 'Missing required fields: code, user, unit, isAdmin, activation, createdBy' 
            });
        }
