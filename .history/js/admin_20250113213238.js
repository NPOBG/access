...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!fromKeypad) {
        try {
            ...
            // Special case for default admin code
            if (!userCode && currentUser === '1234') {
                const adminCode = codes.find(code => code.code === '1234');
                if (adminCode && adminCode.isActive) {
                    const now = new Date();
                    const activation = new Date(adminCode.activation);
                    const deactivation = adminCode.deactivation ? new Date(adminCode.deactivation) : null;
                    
                    if (now >= activation && (!deactivation || now <= deactivation)) {
                        userCode = {
                            user: 'Admin',
                            type: 'Admin',
                            isAdmin: true
                        ...
                        alert('Admin code is not currently active');
                        window.location.href = 'index.html';
                        return;
                    }
                } else {
                    alert('Admin code not found or inactive');
                    window.location.href = 'index.html';
                    return;
                }
            }
            ...
            if (!userCode) {
                alert('Access denied. No access code found for current user.');
                window.location.href = 'index.html';
                return;
            }
