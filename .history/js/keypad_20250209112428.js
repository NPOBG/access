class Keypad {
    ...
    async submitCode() {
        try {
            ...
            if (result.error) {
                // If not changing admin code, show error
                if (!this.isChangingAdminCode) {
                    alert(result.error);
                    this.currentCode = '';
                    this.updateDisplay();
            ...
            // If this is a default admin code and it's valid
            if (result.isAdmin && !this.isChangingAdminCode) {
            ...
            if (this.isChangingAdminCode) {
                if (!this.newAdminCode) {
                    ...
                    const updateResponse = await fetch('/api/verify', {
                        ...
                        body: JSON.stringify({
                            code: result.code, // Use the original admin code
