...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (sessionData) {
        try {
            ...
            const requiredProps = ['code', 'expires', 'type', 'timestamp', 'user', 'isAdmin'];
            for (const prop of requiredProps) {
                if (!session[prop]) {
                    throw new Error(`Invalid session structure - missing ${prop}`);
                }
            }
            
            // Check expiration with buffer
            const expirationBuffer = 30000; // 30 seconds buffer
            if (now >= (session.expires - expirationBuffer)) {
                throw new Error('Session expired');
            }
            
            // Validate admin type
            if (session.type !== 'Admin' || !session.isAdmin) {
                throw new Error('Invalid user type');
            }
            
            // Verify session storage items match
            const storedUser = sessionStorage.getItem('adminUser');
            const storedCode = sessionStorage.getItem('adminCode');
            const storedSession = sessionStorage.getItem('adminSession');
            
            if (!storedUser || !storedCode || !storedSession) {
                throw new Error('Session verification failed - missing storage items');
            }
            
            // Verify all session data matches
            const parsedStoredSession = JSON.parse(storedSession);
            if (storedUser !== session.user || 
                storedCode !== session.code ||
                parsedStoredSession.expires !== session.expires ||
                parsedStoredSession.timestamp !== session.timestamp) {
                throw new Error('Session verification failed - data mismatch');
            }
            ...
            const newSession = {
                ...session,
                expires: now + (8 * 60 * 60 * 1000) // Extend session by 8 hours
            };
