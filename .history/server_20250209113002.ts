...
app.post('/api/verify', async (req: Request, res: Response) => {
  try {
    ...
    if (newCode && confirmCode) {
      // First validate the current admin code
      const adminCode = await AccessCode.findOne({ code: code, isAdmin: true });
      if (!adminCode) {
        return res.status(401).json({ error: 'Invalid admin code' });
      }

      // Then validate the new codes match
      if (newCode !== confirmCode) {
        return res.status(400).json({ error: 'New codes must match' });
      }
      ...
      await AccessCode.deleteOne({ code: code });
