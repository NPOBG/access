...
export type AccessCodeType = 'Admin' | 'Host' | 'Resident' | 'Guest';
...
const typeHierarchy: Record<AccessCodeType, number> = {
    'Admin': 4,
    'Host': 3,
    'Resident': 2,
    'Guest': 1
...
accessCodeSchema.methods.canManage = function(otherCode: IAccessCode): boolean {
    ...
    if (typeHierarchy[this.type as AccessCodeType] <= typeHierarchy[otherCode.type]) {
        return false;
    }
    ...
    if (this.type === 'Host') {
        return this.properties?.some((p: string) => 
            otherCode.properties?.includes(p) || 
            otherCode.units?.some((u: string) => this.units?.includes(u))
        );
    }
    ...
    if (this.type === 'Resident') {
        return otherCode.type === 'Guest' && 
               this.units?.some((u: string) => otherCode.units?.includes(u));
    }
