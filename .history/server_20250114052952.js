...
app.post('/api/access-codes', (req, res) => {
    try {
        ...
        if (!code || !user || !unit || typeof isAdmin === 'undefined') {
            return res.status(400).json({ 
                error: 'Missing required fields: code, user, unit, isAdmin' 
            });
        }
        ...
        if (data.accessCodes.some(ac => ac.code === code)) {
        ...
        data.accessCodes.push(newAccessCode);
        writeData(data);
